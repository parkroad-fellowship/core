<div>
    <div class="card" id="pledgeview" wire:show="!submitted">
        ...
        <form wire:submit="submit">
            <x-turnstile :for-livewire="true" />
            <div class="ferr" role="alert" aria-live="assertive">
            ...
            <input type="email" id="f_email" placeholder="you@example.com (optional, for updates)"
                autocomplete="email" wire:model.blur="email">
            ...
            <input type="tel" id="f_phone" placeholder="e.g. 0712 345 678 (optional)" autocomplete="tel"
            ...
            <input type="number" id="f_amount" placeholder="e.g. 50000" min="1" step="1"
            ...
            <div class="freq-grid" role="radiogroup" aria-label="Pledge frequency">
                @foreach ($this->frequencies() as $option)
                    <button type="button" class="freq-opt @if ($frequency === $option['value']) sel @endif" role="radio"
            ...
            @if ($this->impact())
                <div class="impact">
                    ...
                    <div class="small">
                        @if ($frequency === 0) A one-time annual gift.
                        @else
                            {{ number_format((float) $amount) }} every @if ($frequency === 1)
                                month
                            @else
                                {{ $frequency }} months
                            @endif. @endif
    ...
    @script
        <script>
            window.onPledgeTurnstile = (token) => {
                $wire.set('turnstileToken', token);
            };

            window.onPledgeTurnstileExpired = () => {
    ...
    <div class="card confirm" id="confirmview" wire:show="submitted" style="display:none;">
        ...
        <p>Your pledge is recorded. @if (filled($summary['email'] ?? null))
                We'll send a confirmation to <b>{{ $summary['email'] }}</b>.
            @else
                If you left an email, we'll send you a confirmation there.
            @endif
        </p>
        <div class="sum">
        ...
        <button class="btn2" type="button" wire:click="startOver" style="margin-top:22px;">Make another
            pledge</button>
    </div>
</div>
