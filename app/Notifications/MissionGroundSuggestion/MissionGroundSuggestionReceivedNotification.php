...
class MissionGroundSuggestionReceivedNotification extends BaseNotification
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        return new MailMessage()
            ...
            ->when($missionGroundSuggestion->description, function ($mail) use ($missionGroundSuggestion) {
                return $mail->line('**Additional Notes:**')->line($missionGroundSuggestion->description)->line('');
            })
            ->line('**Next Steps:**')
            ...
            ->line('')
            ->action('📋 Review Suggestion', route(
                'filament.admin.resources.mission-ground-suggestions.edit',
                $missionGroundSuggestion->id,
            ))
            ->line('')
            ->line(
                'This suggestion was submitted on ' . $missionGroundSuggestion->created_at->format('F j, Y \a\t g:i A'),
            )
            ->line('')
