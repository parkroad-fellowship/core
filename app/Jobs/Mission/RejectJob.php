...
class RejectJob
{
    ...
    public function handle(): Mission
    {
        ...
        return DB::transaction(function () use ($reason): Mission {
            throw_unless(
                $this->mission->status->canMoveTo(PRFMissionStatus::REJECTED),
                InvalidStateTransition::class,
                'This mission is ' . strtolower($this->mission->status->getLabel()) . ', so it cannot be rejected.',
            );
            ...
            $this->mission->status->moveTo(PRFMissionStatus::REJECTED);
