...
class GenerateWeatherRecommendationsJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $forecast = WeatherForecast::query()
            ->where('mission_id', $mission->id)
            ->whereDate('forecast_date', $mission->start_date)
            ->first();
        ...
        *  `activities`: (JSON array of strings with specific, weather-related gospel outreach activities)
        ...
        $decodedText = json_decode($json, true);

        Mission::query()
            ->where('id', $mission->id)
            ->update([
                'dressing_recommendations' => collect($decodedText['recommendations'][0]['dressing'])->join("\n"),
                'activity_recommendations' => collect($decodedText['recommendations'][0]['activities'])->join("\n"),
                'weather_recommendations' => $decodedText['recommendations'][0],
            ]);
    }
}
