...
it('initializes Paystack transaction', function () {
    ...
    expect($result['status'])
        ->toBeTrue()
        ->and($result['data']['authorization_url'])
...
it('verifies Paystack transaction', function () {
    ...
    expect($result['status'])->toBeTrue()->and($result['data']['status'])->toBe('success');
...
it('embeds content via NLP service', function () {
    ...
    Http::assertSent(function ($request) {
        return str_contains($request->url(), 'embedding/init') && str_contains($request->body(), 'test document 1');
    });
});
...
it('computes route via Google Maps', function () {
    ...
    $result = $maps->computeRoute(['latitude' => -1.29, 'longitude' => 36.77], [
        'latitude' => -1.30,
        'longitude' => 36.78,
    ]);
...
it('transcribes audio via Azure Speech', function () {
    ...
    $result = $stt->transcribe(['https://example.com/audio.wav'], 'Test Audio');

    expect($result['status'])->toBe('Running')->and($result['self'])->toContain('transcriptions/123');
...
it('gets transcription status via Azure Speech', function () {
    ...
    $stt = new AzureSpeechService();
    $result = $stt->getTranscriptionStatus('https://test-region.api.cognitive.microsoft.com/transcriptions/123');
...
it('returns empty array on failed Azure transcription files', function () {
    ...
    $stt = new AzureSpeechService();
    $result = $stt->getTranscriptionFiles('https://test-region.api.cognitive.microsoft.com/transcriptions/123/files');
